    ...
    try {
        ...
        document.querySelectorAll('.heart-icon').forEach(icon => {
            icon.addEventListener('click', (e) => {
                ...
                if (favoritos.some(fav => fav.id === cantoId)) {
                    ...
                    alert('Canto agregado a favoritos:', canto);
                }
                saveFavoritos();
        ...
        document.querySelectorAll('.play-icon').forEach(icon => {
            icon.addEventListener('click', (e) => {
                const cantoId = e.target.getAttribute('data-id');
                const canto = data.cantos.find(c => c.id === cantoId);
                if (presentar.some(pre => pre.id === cantoId)) {
                    presentar = presentar.filter(pre => pre.id !== cantoId);
                    e.target.classList.remove('present');
                } else {
                    presentar.push(canto);
                    e.target.classList.add('present');
                    alert('Canto agregado a presentación:', canto);
                }
                savePresentar();
...
export function loadPresentar() {
    ...
    const titles = presentar.map(canto => `
        <li data-id="${canto.id}">
            ${canto.id} - ${canto.titulo}
            <span class="play-icon present" data-id="${canto.id}">▶</span>
        </li>
    `).join('');
